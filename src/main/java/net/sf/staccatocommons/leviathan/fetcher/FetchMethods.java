...
public class FetchMethods {
  ...
  public static FetchMethod fromGet(final URI uri, final Map<String, Object> context) {
    return new FetchMethod() {
      @Override
      public void fetch(AsyncUriFetcher fetcher, Closure<URIFetcherResponse> closure) {
        fetcher.get(new InmutableURIAndCtx(uri, context), closure);
      }
    };
  }
  ...
  public static FetchMethod fromPost(final URI uri, final Resource resource) {
    return new FetchMethod() {
      ...
      public void fetch(AsyncUriFetcher fetcher, Closure<URIFetcherResponse> closure) {
        try {
          fetcher.post(new InmutableURIAndCtx(uri), resource.getInputStream(), closure);
        } catch (IOException e) {
